...
\begin{frame}{Why dynamic rock testing needs more than UCS}
  \begin{exampleblock}{Core purpose}
    Link classical Hopkinson-bar theory, six laboratory loading modes,
    controlled experiments, and damage-model validation in one clear sequence.
  \end{exampleblock}
  \vspace{0.25cm}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
...
\begin{frame}{One laboratory platform, six loading modes}
  \begin{columns}[T,onlytextwidth]
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Loading setup}
        ...
        Static pre-stress: $\sigma_x=30$, $\sigma_y=20$,
        $\sigma_z=15$ MPa. Delays are programmable; the six-mode damage
        comparison uses $\Delta t_y=80~\mu$s and $\Delta t_z=160~\mu$s.
      \end{exampleblock}
    \end{column}
  \end{columns}
  \vspace{0.2cm}
...
\section{Six-Mode Similarities and Differences}

\begin{frame}{Six-mode comparison: what is shared and what changes}
  \tiny
  \begin{tabular}{@{}p{0.08\textwidth}p{0.13\textwidth}p{0.21\textwidth}p{0.25\textwidth}p{0.17\textwidth}@{}}
    ...
    4 & EM X & Static triaxial pre-stress & Half-sine amplitude-duration control & Same pre-stress as Modes 5--6 \\
    5 & EM X/Y/Z & Timed multi-axis pulses & Loading order bends the stress path & Same EM $A,\tau$ as Mode 4 \\
    6 & EM $\pm$ axes & Symmetric squeeze & Hydrostatic compaction and force cancellation & Same EM $A,\tau$ \\
    \bottomrule
  \end{tabular}
  \vspace{0.16cm}
...
\begin{frame}{Six-mode result summary: similarities and differences}
  ...
  \begin{tabular}{@{}p{0.10\textwidth}p{0.24\textwidth}p{0.29\textwidth}p{0.27\textwidth}@{}}
    ...
    4 & Same $30/20/15$ MPa pre-stress as Modes 5--6 & One EM X pulse, 400 MPa peak, 200 $\mu$s & Pulse amplitude and duration are explicit inputs \\
    5 & Same EM amplitude/duration as Modes 4 and 6 & Programmable X/Y/Z timing & Curved path shows loading-order and path dependence \\
    6 & Same EM pulse scale as Modes 4 and 5 & Opposed symmetric pulses on $\pm X,\pm Y,\pm Z$ & Hydrostatic compaction dominates; momentum cancels \\
    \bottomrule
  \end{tabular}
\end{frame}
...
\begin{frame}{Focused comparison: traditional, true-triaxial, and symmetric loading}
  ...
  \begin{block}{Normalised input for the focused comparison}
    Same sandstone specimen, same static pre-stress
    $\sigma_{x0}/\sigma_{y0}/\sigma_{z0}=30/20/20$ MPa, and the same
    half-sine stress-wave scale $A=400$ MPa, $\tau=200~\mu$s.
  \end{block}
  \vspace{0.10cm}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{Controlled experiment rule}
        Use the same sandstone sample geometry and material constants in all
        six modes. When a mode has pre-static stress, use the default
        laboratory values from the integrated analysis setup.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Wave-input rule}
        \begin{itemize}
          ...
          \item EM modes use the same half-sine amplitude and duration.
          \item Mode~5 may change delay to expose path dependence.
        \end{itemize}
      \end{exampleblock}
    \end{column}
  \end{columns}
...
\begin{frame}{Step 4: six-mode damage-model result}
...
\begin{frame}{Step 4: six-mode damage descriptors}
  \tiny
  \begin{tabular}{@{}p{0.08\textwidth}p{0.34\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.09\textwidth}p{0.09\textwidth}@{}}
    ...
    6 & EM $\pm$XYZ; same $A,\tau$ per opposed bar & 0.00 & -- & 0.00 & 0.00 \\
    \bottomrule
  \end{tabular}
  \vspace{0.22cm}
  \begin{block}{Main message}
    The six modes use comparable material inputs, but they do not create the
    same failure mechanism: Modes~1--5 drive shear failure, while Mode~6
...
\begin{frame}{Closing}
  \begin{center}
    ...
    \normalsize The six modes turn uniaxial strength, in-situ stress,
