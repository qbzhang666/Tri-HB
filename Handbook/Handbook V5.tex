...
% ---------------- Bibliography -------------------------------------------------
\addbibresource{handbook_trihb V5.bib}
...
\vfill
{\large First Edition \textbullet\ Version 5}\\[6pt]
{\normalsize \today}
\end{titlepage}
...
First edition, Version 5 (aligned with the four-family
\texttt{tri\_hb\_integrated.py} workspace).
...
(file \texttt{tri\_hb\_integrated.py}) that bundles seven analysis steps into
...
experimental validation; (5) a publication-ready summary of all results;
(6) generation of a presentation deck (LaTeX + PDF) from the current run; and
(7) generation of a short run report (LaTeX + PDF). The workspace is described
in detail in Chapters~\ref{ch:integrated}--\ref{ch:wavedamage}.
...
application, \texttt{tri\_hb\_integrated.py}, that bundles seven analysis
...
and the Step~6--7 document generators are rendered by the integrated file
itself. The direct command
...
\begin{description}[leftmargin=2em,style=nextline]
  ...
  \item[Step 6: Presentation]
    Generates a \emph{new} Beamer slide deck (LaTeX source plus a compiled
    PDF) from the current run's Steps~1--5, styled like the Tri-HB Handbook
    lecture deck. Each figure slide places the figure beside a short
    plain-language description and its governing equations; title, overview,
    test-configuration, and per-step key-result slides are generated
    automatically. The page offers the PDF, the editable \texttt{.tex}, and a
    self-contained ZIP (\texttt{.tex} plus figures) so the deck can be
    recompiled anywhere. It uses the Handbook deck only as a visual template;
    the content is the current session, not the static Handbook file.
  \item[Step 7: Report]
    Generates a \emph{new} short report (LaTeX source plus a compiled PDF)
    that summarises the current run's Steps~1--5: an overview, a
    test-configuration table, a grouped key-results table, the captured
    figures with their descriptions and equations, and a scope note. As in
    Step~6, the PDF, the \texttt{.tex}, and a ZIP bundle are offered. Both
    generators reuse the exact metrics and \SI{600}{dpi} figures captured on
    the Step~5 summary page, so the documents match what was shown there.
\end{description}

\noindent Both Step~6 and Step~7 require the Step~5 summary to have been
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item Open Step~5 to capture the publication-quality figure set, then use
        Step~6 (presentation) and Step~7 (report) to generate ready-to-share
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
  ...
  \item Step 5: open \emph{Summary of Results} to capture every step's key
        ...
        the metrics and figures that Steps~6--7 reuse.
  \item Step 6: open \emph{Presentation} to generate a Beamer slide deck of
        the run; download the compiled PDF, the editable \texttt{.tex}, or the
        ZIP bundle.
  \item Step 7: open \emph{Report} to generate the short run report; download
        the compiled PDF, the \texttt{.tex}, or the ZIP bundle.
\end{enumerate}
\end{stepbox}
...
\section{Document generation: Steps~6 and~7}\label{sec:docgen}

The final two workspace pages turn the current run into ready-to-share
documents. Both reuse the metrics table and the \SI{600}{dpi} figure set that
the Step~5 summary captures in session state (each figure is stored together
with its caption, its short reading guide, and the \LaTeX{} of its governing
equations), so the generated documents match the Step~5 view exactly.

\begin{description}[leftmargin=2em,style=nextline]
  \item[Step 6: Presentation] Builds a Beamer slide deck in the Handbook
  ...
  \item[Step 7: Report] Builds a short article-class report: an overview, a
    test-configuration table, a grouped key-results table, the captured
...
Each page offers three downloads: the compiled PDF, the editable \texttt{.tex}
