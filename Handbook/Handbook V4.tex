...
The Tri-HB facility documented in this handbook supports six physics branches:
...
\noindent In the current Streamlit interface these six physics branches are
presented as four top-level loading families:
\emph{Gas-gun uniaxial SHPB}, \emph{Confinement-chamber SHPB},
\emph{Gas-gun Tri-HB static--dynamic loading}, and
\emph{Electromagnetic programmable loading}.  The electromagnetic family
then exposes a secondary topology selector: \emph{single-axis, one-sided}
(legacy Mode~4), \emph{multi-axis, one-sided} (legacy Mode~5), and
\emph{symmetric opposing pairs} (legacy Mode~6).  The exported
configuration retains the machine-readable fields
\texttt{loading\_family}, \texttt{em\_topology}, \texttt{active\_axes},
and \texttt{legacy\_mode}, so the cleaner interface remains traceable to
the handbook equations.
...
(file \texttt{tri\_hb\_integrated.py}) that bundles five analysis steps into
...
experimental validation; and (5) a publication-ready summary of all results.
The workspace is described in detail in
...
application, \texttt{tri\_hb\_integrated.py}, that bundles five analysis
...
is rendered by the integrated file itself. The direct command
