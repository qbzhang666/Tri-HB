...
steel has a proportional limit $\sigprop \approx \SI{1000}{MPa}$. Whenever the
...
\[
  \boxed{\;\sig_{\text{bar}}^{\max} \leq \sigprop = \SI{1000}{MPa}\;}
\]
...
calibrated against the hydraulic loading frame currently in the computational workspace
...
\begin{itemize}[leftmargin=*, itemsep=4pt]
  ...
  \item \textbf{$\sigma_2$ and $\sigma_3$ are confining stresses.} Because Y
    ...
    \begin{equation}
      ...
      \left(\frac{\epsdot}{\epsdot_{\text{ref}}}\right).
    \end{equation}
        Here $b_{\text{rate}}$ is the fitted slope of strength increase versus
        $\log_{10}\epsdot$, and $\epsdot_{\text{ref}}$ is the reference strain
        rate where $\text{DIF}=1$. The simulator presets use
        $\epsdot_{\text{ref}}=\SI{1e-5}{s^{-1}}$.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Static pre-stress range (current computational workspace):
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Pre-stress}: servo-controlled hydraulic cylinders apply
        $\sig_1$, $\sig_2$, $\sig_3$ independently. The current computational workspace
...
\begin{enumerate}[leftmargin=*, itemsep=3pt]
  ...
  \item Plot $\sig_x^{\text{total}}$ versus $\eps_x$. Read peak strength and
        average strain rate.
  \item Check that maximum bar stress remained below $\sigprop = \SI{1000}{MPa}$.
\end{enumerate}
\end{stepbox}
...
\textbf{Conclusion:} \SI{706}{MPa} is below the \SI{1000}{MPa} bar limit.
Safe to proceed, but with reduced headroom. Charge the capacitor for
$\sigpeak \approx \SI{800}{MPa}$
(margin above 706 to drive the specimen through peak), choose
$\tau = \SI{200}{\micro s}$ to give a strain rate of order \SI{200}{s^{-1}}.
The simulator will report the total axial stress reaching $\sim$\SI{726}{MPa}
...
though it would require $\sigpeak = \SI{750}{MPa}$ (closer to the bar limit) in
uniaxial mode.
...
the current computational workspace's calibration. The slider maximum is set in one place
...
\begin{description}[leftmargin=2em, style=nextline]
  ...
  \item[Specimen Stresses] Total $\sig_X$, $\sig_Y$, $\sig_Z$ on the specimen.
    ...
    reference. A horizontal dashed line at \SI{1000}{MPa} shows the bar
...
Step~3 uses the total stress history in each loading direction:
\begin{align}
  \sig_x(t) &= \sig_{x0} + \Delta\sig_x(t), \\
  \sig_y(t) &= \sig_{y0} + \Delta\sig_y(t), \\
  \sig_z(t) &= \sig_{z0} + \Delta\sig_z(t).
\end{align}
The prestress terms set the starting point, while the dynamic increments come
from the finite-duration pulse histories generated in the wave model. Assuming
the principal axes align with the bar axes, the diagonal Cauchy tensor gives the
invariants
\begin{align}
...
The constants $A$, $B$, and $n$ are empirical pressure-dependent failure-envelope
parameters that should be fitted to triaxial strength data or a calibrated DEM
failure surface. The Lode-angle factor $a_\theta$ should be fitted from
true-triaxial or compression/extension data; set $a_\theta=0$ when no
Lode-angle calibration is available.
...
$F > 1$).
The damage constants $\tau_D$, $F_0$, $\alpha$, $m$, $\beta$, and
$\epsdot_0$ are calibration parameters, not directly known from the wave-speed
calculation. They should be fitted against dynamic strength, pulse-duration
effects, dissipated energy, CT/DIC damage measures, or DEM bond-breakage
histories. The app defaults are planning values for sensitivity checks.
...
\paragraph{Delay-sensitivity indicators.}
The delay-sensitivity plot uses the normalised delay
$\Delta t^\ast=\Delta t/t_{\text{travel}}$ and recomputes the centre-stress
superposition factor for each candidate delay:
\begin{equation}
  \eta_{\text{sup}}(\Delta t^\ast) =
    \frac{\max_t \sig_{\text{centre}}(t;\Delta t^\ast)-\sig_{x0}}{A_x}.
\end{equation}
The two smooth control functions used by the sensitivity indicators are
\begin{align}
  C_{\text{wave}} &= \exp\!\left[-(\Delta t^\ast/1.2)^2\right], \\
  C_{\text{damage}} &=
    \frac{1}{1+\exp\!\left[-(\Delta t^\ast-5.0)/1.3\right]}.
\end{align}
The plotted final-damage and central-damage curves are normalised heuristic
indicators,
\begin{align}
  I_{\text{final}} &=
    \operatorname{clip}\!\left(0.15 + 0.35C_{\text{wave}}
    + 0.25C_{\text{damage}},\,0,\,1\right), \\
  I_{\text{central}} &=
    \operatorname{clip}\!\left(0.75C_{\text{wave}}
    + 0.25(1-C_{\text{damage}}),\,0,\,1\right).
\end{align}
$\eta_{\text{sup}}$ is a superposition ratio and can exceed one; it is not a
damage variable. $I_{\text{final}}$ and $I_{\text{central}}$ are planning
indicators for comparing delays, not independent calibrated damage predictions.
...
can be modelled.
$b_{\text{rate}}$ and $\epsdot_{\text{ref}}$ are calibration constants for the
dynamic increase factor. They should be obtained by fitting peak strength data
measured at several strain rates under comparable confinement:
\begin{equation}
  b_{\text{rate}} =
  \frac{\text{DIF}_2-\text{DIF}_1}
       {\log_{10}(\epsdot_2/\epsdot_{\text{ref}})
        -\log_{10}(\epsdot_1/\epsdot_{\text{ref}})},\qquad
  \text{DIF}_i=
  \frac{\sig_{\text{peak},i}}
       {\sig_{c0}+k_{\text{conf}}\sig_{\text{conf}}}.
\end{equation}
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
  ...
  \item In Mode 4 (Symmetric XYZ), find the peak specimen pressure achievable
        with single-pulse $\sigpeak = \SI{1000}{MPa}$ (right at the bar limit).
