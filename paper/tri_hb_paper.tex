...
%  Integrated Computational Workspace for Multiaxial Dynamic Rock Characterisation
%
...
and an Integrated Computational Workspace for Multiaxial Dynamic Rock Characterisation}
...
blasting; computational workspace; gradient damage; phase-field fracture; CT
calibration.
...
(\texttt{tri\_hb\_integrated.py}) and the figure-generation scripts are part of the
Tri-HB repository. Figures~1--8 are produced from prescribed-pulse reference cases
by the workspace's analysis pipeline. Figure~\ref{fig:spatial} (§\ref{sec:spatial})
demonstrates the spatial framework on the same reference case; CT scan images,
bar-signal records, fracture surface area measurements, and high-speed-camera data
from the Tri-HB campaign are available from the corresponding author on reasonable
request.\\[2pt]
