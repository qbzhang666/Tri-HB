...
% ---------------- Bibliography -------------------------------------------------
\addbibresource{handbook_trihb.bib}
...
treats each of the six loading modes in turn, deriving the relevant wave
...
\begin{longtable}{@{}p{3.2cm}p{10cm}p{2cm}@{}}
\toprule
...
The Tri-HB facility documented in this handbook supports six loading modes:

\begin{enumerate}[leftmargin=2.4em,itemsep=4pt]
\item \textbf{Mode 1 -- Gas-gun uniaxial} (classical SHPB). A pneumatically
      launched striker bar strikes the incident bar, producing a roughly
      trapezoidal pulse shaped by a copper or rubber pulse shaper. The
      specimen is loaded along a single axis, with no active confining
      pressure.
\item \textbf{Mode 2 -- Confinement-chamber SHPB}. A cylindrical specimen
...
\item \textbf{Mode 3 -- Monash Tri-HB}: gas-gun coupled static--dynamic
      triaxial loading. Same gas-gun X-axis hardware as Modes~1 and~2, but
      ...
      of cubic specimens.
\item \textbf{Mode 4 -- Electromagnetic uniaxial}. A capacitor-bank-driven
      coil generates a clean half-sine pulse on a single bar, with
      independently tunable amplitude and duration.
\item \textbf{Mode 5 -- Electromagnetic asynchronous triaxial}. Three
      orthogonal pulses fire with programmable inter-axis delays, exposing
      the path-dependence of dynamic failure.
\item \textbf{Mode 6 -- Electromagnetic symmetric multidirectional}.
      Opposing bar pairs fire simultaneously, constructively superposing in
      force while cancelling in momentum and doubling the achievable
      specimen stress without exceeding the bar elastic limit.
\end{enumerate}

\noindent All six modes permit independent static pre-stress on the three
principal axes (axisymmetric in Mode~2; fully independent in Modes~3--6),
in the range \SIrange{0}{50}{MPa} per axis for the current Monash digital
twin.
...
(file \texttt{tri\_hb\_integrated.py}) that bundles four analysis steps into
...
(3) stress-path and invariant analysis; and (4) damage evolution and DEM/
experimental validation. The workspace is described in detail in
Chapters~\ref{ch:integrated}--\ref{ch:wavedamage}.
...
investigated systematically. The six loading modes described above are
...
This chapter develops the equations common to all six loading modes. The
...
$\nu_b$. The simulator's default of an unshaped rectangular pulse contains
arbitrarily high $ka$ harmonics and is therefore prone to spurious
oscillations as the pulse propagates; the half-sine pulse of the
electromagnetic mode and the trapezoidal shape of the pulse-shaped gas-gun
mode both concentrate energy at $ka \ll 1$ and are essentially
dispersion-free for the geometries used here.
...
\begin{figure}[htbp]
  ...
  \caption{Mode~1 sample run: sandstone, gas-gun uniaxial SHPB, $V=\SI{20}{m/s}$, no static confinement.}
...
\end{align*}
A $\SI{20}{m/s}$ impact thus produces an approximately rectangular pulse of
\SI{406}{MPa} amplitude lasting \SI{193}{\micro s}.
\end{stepbox}
...
approximates the shaped pulse with a trapezoidal envelope having
\SI{20}{\micro s} rise/fall and a flat plateau between.
...
\cite{ChenSong2011,XiaYao2015,Zhou2010}. A cylindrical specimen is held
...
\begin{figure}[htbp]
  ...
  \caption{Mode~2 sample run: sandstone in a confinement chamber, $V=\SI{20}{m/s}$, $\sig_1=\SI{30}{MPa}$ and $p_c=\SI{20}{MPa}$.}
...
\begin{figure}[htbp]
  ...
  \caption{Mode~3 sample run: sandstone with independent static triaxial pre-stress and gas-gun dynamic loading, $V=\SI{20}{m/s}$.}
...
incident bar. The data-reduction equations are identical to Mode~1; the
distinguishing feature is the pulse shape and the independent control of
amplitude and duration.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/handbook_modes/mode_4_em_uniaxial.png}
  \caption{Mode~4 sample run: sandstone, single electromagnetic half-sine pulse on $+X$, $\sigpeak=\SI{400}{MPa}$ and $\tau=\SI{200}{\micro s}$.}
...
Where the gas-gun pulse is trapezoidal, the electromagnetic generator
produces a clean half-sine pulse:
\begin{empheq}[box=\fbox]{equation}
...
This mode uses three of the six electromagnetic generators to apply pulses
along the $+X$, $+Y$, and $+Z$ axes, each with an independent time delay.
It enables the study of how the \emph{order} of multi-axis loading affects
material failure --- a question that classical uniaxial Hopkinson testing
cannot address.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/handbook_modes/mode_5_em_async_triaxial.png}
  \caption{Mode~5 sample run: sandstone, electromagnetic pulses on $+X$, $+Y$, and $+Z$ with \SI{80}{\micro s} and \SI{160}{\micro s} inter-axis delays.}
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
  \item Apply static pre-stress $\sig_1^{0}$, $\sig_2^{0}$, $\sig_3^{0}$.
        Record baselines.
  \item Choose pulse delays $\Delta t_y$, $\Delta t_z$ relative to
        $\Delta t_x=0$. Typical values: $\Delta t=0$ (synchronous, ground
        truth), $\Delta t=\tau/4$ (mild asynchrony), $\Delta t=\tau/2$
        (strong asynchrony).
...
facility. All six electromagnetic generators fire \emph{simultaneously},
delivering identical pulses on the opposing bar pairs $(+X,-X)$, $(+Y,-Y)$,
$(+Z,-Z)$. The two pulses on each axis arrive at the specimen at the same
instant, superposing constructively in force while cancelling in momentum.
The specimen is compressed from all sides simultaneously.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/handbook_modes/mode_6_em_symmetric_xyz.png}
  \caption{Mode~6 sample run: sandstone, full symmetric XYZ electromagnetic operation, $\sigpeak=\SI{400}{MPa}$ per single bar and $\tau=\SI{200}{\micro s}$.}
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
  \item Apply static pre-stress: set $\sig_1^{0}=\sig_2^{0}=\sig_3^{0}$ for
        the hydrostatic sub-mode, or independent values otherwise.
...
%==============================================================================
\chapter{Comparison of the six loading modes}\label{ch:comparison}
%==============================================================================
...
\midrule
Pulse shape & Trap.\ (shaped) & Trap.\ (shaped) & Trap.\ (shaped) & Half-sine & Half-sine $\times 3$ & Half-sine $\times 6$ \\
...
Active dynamic bars & 1 (X) & 1 (X) & 1 (X) & 1 (X) & 3 ($+X,+Y,+Z$) & Up to 6 ($\pm$XYZ) \\
Lateral architecture & None & Pressure chamber & 4 output bars & 4 output bars & 4 output bars & 4 output bars \\
Confining stress & None & $\sig_2{=}\sig_3{=}p_c$ axisym. & $\sig_2,\sig_3$ indep.\ & $\sig_2,\sig_3$ indep.\ & $\sig_2,\sig_3$ indep.\ & $\sig_2,\sig_3$ indep.\ \\
Default $\sig_1,\sig_2,\sig_3$ & 0, 0, 0 & 30, 20, 20 & 30, 20, 15 & 30, 20, 15 & 30, 20, 15 & 30, 20, 15 \\
...
Lode angle reach & $\theta=0$ & $\theta=0$ fixed & $\theta\in[0°,60°]$ & $\theta=0$ & $\theta\in[0°,60°]$ & $\theta\in[0°,60°]$ \\
Stress-path control & Uniaxial only & Static triax.\ (axisym.) & Static triax.\ + dyn.\ axial & Uniaxial only & Yes (async) & Hydrostatic / biaxial \\
Pure hydrostatic? & No & No & No & No & Approx.\ if $\Delta t{=}0$ & Yes \\
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/handbook_modes/mode_comparison_summary.png}
  \caption{One-sample visual comparison of the six Tri-HB loading modes, using the same sandstone reference case to show how pulse source, confinement, stress path, and damage indicators change across the apparatus configurations.}
  \label{fig:mode-comparison-summary}
\end{figure}
...
\begin{itemize}[leftmargin=*,itemsep=3pt]
  ...
  \item \textbf{Mode-3 misinterpretation.} A uniaxial constitutive model
...
wave, stress-path, damage, energy, and validation pages. The direct command
...
step reads the same Streamlit session-state entries:
\begin{center}
  \texttt{tri\_hb\_latest\_config}\\
  \texttt{tri\_hb\_latest\_result}
\end{center}

\begin{description}[leftmargin=2em,style=nextline]
  \item[Overview] Landing page that explains the workflow. Use it to
    confirm the handbook matches the deployed application version.
  ...
  \item[Step 4: Damage model and DEM validation]
    ...
    Same module, ``Damage evolution'' tab first.
\end{description}
...
\begin{description}[leftmargin=2em,style=nextline]
  ...
  \item[Test loading]
    Loading mode (6 modes), symmetric-axis selector (Mode~6), pulse
    settings (striker velocity for the gas gun; $\sigpeak$ and $\tau$ for
    EM), static pre-stress sliders for $\sig_1,\sig_2,\sig_3$, and the
    asynchronous timing sliders if Mode~5 is active.

    \medskip\noindent
    \textbf{Default pre-stress values} (which the sliders populate when
    the loading mode is changed):
    \begin{itemize}[leftmargin=1.5em,itemsep=2pt]
      ...
      \item Modes~4--6 (electromagnetic): same independent defaults as
            Mode~3, $\sig_1=\SI{30}{MPa}$, $\sig_2=\SI{20}{MPa}$,
            $\sig_3=\SI{15}{MPa}$.
    \end{itemize}
\end{description}
...
The main panel updates live as inputs change. Five tabs are available;
the fourth ($p$--$\eps_v$) appears only in symmetric mode.

\begin{description}[leftmargin=2em,style=nextline]
  \item[Bar waveforms] Time-history of all strain-gauge signals. The
    X-axis traces $\eps_I,\eps_R,\eps_T$ are always shown when the
    loading mode places dynamic load on X. For Mode~3 (Monash Tri-HB),
    ...
    and only the X-axis traces appear. For Mode~5 (asynchronous
    triaxial), three pairs of $\eps_I,\eps_R,\eps_T$ are plotted, one
    pair per active axis. For Mode~6 (symmetric), both $\eps_I^{+}$ and
    $\eps_I^{-}$ are plotted to verify synchronisation.
  \item[Specimen stresses] Total $\sig_X,\sig_Y,\sig_Z$ on the specimen.
    ...
    reference. A horizontal dashed line at \SI{1000}{MPa} indicates the
    bar proportional limit.
  ...
  \item[$p$--$\eps_v$] Visible only in symmetric mode. The compaction
    curve shows the initial bulk-modulus slope followed by the cap
    plateau.
...
Configuration & JSON &
  ...
  warning state, ISO~8601 export timestamp. Provides full provenance. \\
Full workbook & XLSX &
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
  ...
  \item \emph{Experimental setup} tab: \emph{Material preset = granite},
        ...
        settings at default.
  \item \emph{Test loading} tab: \emph{Loading mode = EM Symmetric
        Multi-axis}; pick \emph{Full hydrostatic
        ($\pm X\pm Y\pm Z$)}.
  \item $\sigpeak=\SI{400}{MPa}$, $\tau=\SI{200}{\micro s}$,
...
The failure surface is taken in the Hoek--Brown-like form
\begin{align}
...
outside the static failure surface and damage is allowed to grow.
...
\cite{Perzyna1966,Murakami2012}:
\begin{equation}
  \dot D \;=\; \frac{(1-D)^{\alpha}}{\tau_D}\,
              \left\langle\frac{F-1}{F_0}\right\rangle^{m}
...
only when $F>1$). The exponents and time scale are tabulated in
...
\end{align}
With stresses in MPa and dimensionless strains, $W$ is in
MJ\,m\textsuperscript{-3}.
...
\begin{description}[leftmargin=2em,style=nextline]
  ...
  \item[Analysis goal] \emph{Quick validation}, \emph{Stress-path focus},
    or \emph{Damage calibration}. Pre-fills sensible defaults for the
    chosen workflow.
\end{description}
...
\midrule
$A$ & Failure-surface intercept & \SI{15}{MPa} \\
